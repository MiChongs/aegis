...
控制台启动时会自己判断这一跳并在走公网时打一条 warn（`describeBackendHop`），
因为这个错误在功能上完全看不出来 —— 反代照常工作、接口照常返回，只有客户端 IP
悄悄变成同一个。
