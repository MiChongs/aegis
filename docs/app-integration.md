...
用户当前的功能权益是**仍生效的每一段开通的并集**：先买基础版再买高级版时
两段都没结束，两边的功能都生效；用完的那一段自动出局（先高级版后基础版的「降级」就是这样生效的）。
每一段的功能按套餐**当前**的配置算：运营给套餐加一项功能，老会员立刻能用；
拿掉一项，老会员立刻失去。套餐被删除时，功能定格为删除前的配置。
停用或删除一个功能标识对所有人同时生效；退款冲正的那笔开通不再贡献任何功能。

因此**别把功能列表长期缓存在客户端**：它会随运营改套餐而变。
缓存服务端校验的结论同样按上一节的 `checkedAt` 算 TTL。
