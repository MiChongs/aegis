...
- **报错是中文，且不带 JSON Pointer**。库自带的文案是英文，拼进中文错误里
  读起来是「回调报文不合法：`/: missing properties 'orderNo'`」——
  半中半英，开头那个 `/` 还是根对象的路径。现在是
  「缺少必填字段 "orderNo"、"sign"」「coupon.code 类型应为 string，实际是 number」，
  嵌套路径按脚本里真正会写的形式给（`coupon.code`、`tags[0]`）。
  认不出的错误种类仍旧回落到库的英文原文 —— 那比编一句不准确的中文强。

### 形状校验写在契约里，不要写在脚本里

这是内置模板示范的分工，也是这套东西存在的意义：

| 归谁 | 内容 |
|---|---|
| 入参契约 | 字段在不在、类型对不对、取值在不在枚举里、长度与范围 |
| 脚本 | 契约表达不了的：时间窗、签名、防重放、业务规则 |

在脚本里手写一遍 `aegis.validate.schema(…, ctx.input)` 不是不行，但那要等函数
被真的调起来才生效，而且每个作者都得自己写一遍。契约在**调用入口**就挡掉，
错误里逐条点名 —— 接入方拿到的是「缺少必填字段 orderNo」，不是 `50290`。

### 模板自带契约与样例

每个内置模板都带 `sampleInput`，需要入参的那几个还带 `inputSchema`
（建函数时一并写入）。这一条是补上一次真实反馈的：从「校验第三方回调」
模板建出来的函数，第一次试跑撞上一句
`缺少必填字段 "orderNo"、"amount"、"timestamp"、"sign"` ——
因为试跑输入框里是默认的 `{"action":"ping"}`。模板是大多数人第一次用这套
东西的地方，开箱即报错等于告诉他这功能是坏的。

试跑输入框的起点按「越贴近这个函数真实收到的东西」排序：
**契约现造的样例 → 模板自带的样例 → 空对象**。输入框旁边另有一个
「按契约填」按钮，随时能再造一份。

生成的样例**只填必填字段**（可选字段全填上会让人以为它们是必须的），
并且**保证能通过它自己的契约** —— 有 `minLength` 的字段拿字段名当占位符
补足长度，有 `minimum` 的取下界，有 `enum` 的取第一项。
`pattern` 与 `format` 满足不了：那需要反向生成，而生成不出来的下场是
一个看起来合法、实际过不了的样例，所以那两种约束下交出的仍是占位符。
`TestGeneratedSampleSatisfiesItsSchema` 与
`TestBuiltinTemplateSamplesSatisfyTheirSchema` 钉住这两条。
