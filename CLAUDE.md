...
| **会员与试用** | 会员判定收成一个入口（是不是会员 / 还剩多久 / 是不是**试用** / 试用还能不能领）；试用是套餐的一种，一人一次由唯一约束保证。功能权益**跟随套餐当前配置**（改套餐即对在期会员生效，含降级），退款冲正与扣减会让对应那段会员期不再算数 | [service](internal/service/CLAUDE.md#权益跟随套餐会员段segmentgo--vip_segment_repositorygo) |
