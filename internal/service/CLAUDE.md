...
vip_plans.features         这个套餐包含哪些功能（判定读的是它的当前值）
vip_transactions.features  开通那一刻的功能留档（套餐被删 / 非套餐发放时才用它）
```
...
| 功能**跟随套餐当前配置** | `vipdomain.Segment.EffectiveFeatures` | 见下一小节。快照只在套餐被删、或这笔不是按套餐开的时候生效 |
| 当前权益 = 仍生效的每一段的**并集** | `vipLiveSegmentsSQL` + `vipdomain.Evaluate` | 会员期是顺延的：先买基础版再买高级版时两段都没结束，两边功能都该生效；用完的那几段自然出集合（这也是用户自己「降级」的生效方式） |
| 只在**启用中**的功能目录里取 | `resolveFeatures` | 停用 / 删除一个标识即对所有人同时生效。否则 `/vip/status` 列着一个 `verify` 判不通过的功能，客户端会亮起一个点了没用的入口 |
| 功能权益以「是不是会员」为前提 | `Entitlement.HasFeature` | 过期用户的开通记录仍在账本里，只按标签命中会让到期三个月的人继续用高级功能 |
| 删除功能标识不级联清套餐 | `AdminDeleteFeature` 返回 `affectedPlans` | 删功能是运营动作，不该被"还有套餐在用"卡住；残留引用会在校验入口明确报错，比静默改写一批套餐配置好排查 |

支付直购仍在**下单**时记下功能列表（`MetaKeyVipFeatures`），但它只是留档 ——
权益按套餐当前配置算；只有下单后套餐被删掉的那种订单，才会用上这一份。

#### 权益跟随套餐：会员段（segment.go / vip_segment_repository.go）

功能最早是**按值快照**进账本、判定只读快照的。结果是套餐配置与已开通用户彻底脱钩：
给高级版加一项功能老用户拿不到，把功能拿掉（降级）老用户照用不误 ——
套餐是运营唯一能动的杠杆，它拉不动存量用户，等于这根杠杆是断的。现在：

| 情形 | 这一段的功能取自 |
|---|---|
| 套餐还在（含已下架） | `vip_plans.features` 的**当前值**；套餐改名，「当前套餐」也跟着改 |
| 套餐被删除 | 快照。`DeleteVipPlan` 在同一事务里把仍在期内的记录**定格成套餐最后的配置** —— 否则「先拿掉 export 再删套餐」会让老用户的 export 回来 |
| 自定义发放 / 卡密赠送天数 | 快照（本来就只有这一份，不挂任何套餐） |

一段会员期还有两件事会让它「不再算数」，此前都没有落点：

| 事件 | 以前 | 现在 |
|---|---|---|
| 退款冲正 | 只把 `vip_expire_at` 减掉天数，开通记录原样留着 —— 账上还有别的会员时长时，退掉的高级版照样生效 | `revoked_at` 作废这一段，排在它后面的段整体前移同样天数（`revokeVipSegmentForOrderTx`） |
| 扣减天数（`aegis.vip.revoke`） | 走不通：发放入口拒收负数天数，永远报「发放天数必须大于 0」 | `RevokeVipDays` 从链尾往回截，截断点之后的段整段失效；截过此刻即会员立即结束；记一条负时长的 `admin_revoke` 账 |

`expire_before` / `expire_after` 是账本，**不改写**；段在链上的实际位置单独存
`active_from` / `active_until`，只有上面两件事会动它们。为 NULL 表示从未调整过，
读取端按账本推导 —— 所以迁移不需要回填，滚动发布期间旧实例写的记录也照常生效。
扣减记录不是一段会员期（`duration_days < 0`），不贡献功能、不会成为「当前套餐」。

「当前套餐」「会员来源」取**最近开通**的那一段仍生效的会员期（此前取的是最近一条账，
包括已经退款作废的那条）。卡密开通的会员来源是 `card_key`，此前落进了「来源未知」。

「当前是不是试用」在原来「到期时间恰好等于试用发到的时刻」之外，
再认「最近一段就是试用」—— 扣减过天数的试用前者不再成立。
...
| 错误文案自己翻成中文 | 库自带的是英文，拼进中文错误里是「回调报文不合法：`/: missing properties`」，开头那个 `/` 还是根对象的 JSON Pointer。认不出的种类回落英文原文 —— 比编一句不准确的中文强 |
| 模板自带契约与样例 | 从模板建出来的函数第一次试跑就撞上「缺少必填字段」，是真实反馈。模板是大多数人第一次用这套东西的地方，开箱即报错等于告诉他功能是坏的 |
| 生成的样例必须能过它自己的契约 | 「按契约填」填出来的东西立刻被校验挡回来，比没有这个按钮更让人困惑。有 `minLength` 的拿字段名补足长度，有 `minimum` 的取下界 |

形状校验的分工是：**字段在不在、类型对不对、取值范围**归契约（调用入口就挡掉，
错误逐条点名），**时间窗 / 签名 / 防重放 / 业务规则**归脚本。内置模板按这条改过了 ——
在脚本里手写一遍 `aegis.validate.schema(…, ctx.input)` 要等函数被真的调起来才生效，
而且每个作者都得自己写一遍。
