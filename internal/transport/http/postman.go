...
func buildPostmanRequestItem(path string, method string, op *openapi3.Operation, opts PostmanOptions) (postmanItem, []string, error) {
	...
	description = localizedOperationDescription(method, path, description)
...
func localizedOperationName(method string, path string, op *openapi3.Operation) string {
	...
	if mapped, ok := map[string]string{
		...
		"POST /api/admin/profile/avatar":                  "上传管理员头像",
...
func localizedOperationDescription(method string, path string, description string) string {
	description = strings.TrimSpace(description)
	if description == "" || strings.HasPrefix(description, "Auto-generated reference for `") {
		return fmt.Sprintf("接口说明：`%s %s`。", strings.ToUpper(strings.TrimSpace(method)), path)
	}
	return description
}
...
func translatedPathSegments(path string) []string {
	...
	segmentMap := map[string]string{
		...
		"batch-review":            "批量审核",
		"batch-adjust":            "批量调整",
		"adjust-experience":       "调整经验",
		"adjust-integral":         "调整积分",
		"initialize-user":         "初始化用户",
		"batch-initialize":        "批量初始化",
		"add-users":               "添加用户",
		"remove-users":            "移除用户",
		"send-code":               "发送验证码",
		"verify-code":             "校验验证码",
		"send-password-reset":     "发送重置密码邮件",
		"verify-reset-token":      "校验重置令牌",
		"mobile-login":            "移动端登录",
		"auth-url":                "获取授权地址",
		...
		"levels":                  "等级",
		"level":                   "等级",
		"experience-transactions": "经验记录",
		"integral-transactions":   "积分记录",
		"query":                   "查询",
		"info":                    "信息",
		"start":                   "启动",
		"engine":                  "引擎",
		...
		"verify":                  "校验",
		"categories":              "分类",
		"check-integrity":         "校验完整性",
		"audit-stats":             "审核统计",
		"user-sites":              "用户站点",
		"bulk":                    "批量操作",
		"delete-by-filter":        "按筛选删除",
		"regions":                 "地区",
		"roles":                   "角色",
		"access":                  "访问权限",
		"review":                  "审核",
	...
	fallbackTokenMap := map[string]string{
		"admin": "管理", "app": "应用", "application": "申请", "applications": "申请", "role": "角色",
		"storage": "存储", "payment": "支付", "email": "邮件", "config": "配置", "password": "密码",
		"policy": "策略", "batch": "批量", "adjust": "调整", "experience": "经验", "integral": "积分", "settings": "设置", "auto": "自动",
		"initialize": "初始化", "user": "用户", "users": "用户", "platform": "平台", "send": "发送",
		"verify": "校验", "reset": "重置", "token": "令牌", "auth": "授权", "url": "地址",
		"mobile": "移动端", "create": "创建", "from": "从", "template": "模板", "templates": "模板",
		"save": "保存", "as": "为", "node": "节点", "types": "类型", "type": "类型", "todo": "待办",
		"sign": "签到", "notification": "通知", "notifications": "通知", "read": "已读", "all": "全部",
		"unread": "未读", "count": "数量", "revoke": "撤销", "site": "站点", "daily": "每日",
		"rank": "排行", "ranking": "排行", "security": "安全", "source": "来源", "sources": "来源",
		"trend": "趋势", "channel": "渠道", "preview": "预览", "match": "匹配", "pin": "置顶", "delete": "删除", "filter": "筛选", "audit": "审核", "regions": "地区", "checks": "校验", "integrity": "完整性", "start": "启动", "engine": "引擎", "order": "订单", "orders": "订单", "change": "修改", "categories": "分类", "bulk": "批量", "info": "信息", "apply": "申请",
		"object": "对象", "link": "链接",
	}
	for _, segment := range source {
		...
		if mapped, ok := segmentMap[segment]; ok {
			segments = append(segments, mapped)
			continue
		}
		if strings.Contains(segment, "-") {
			tokens := strings.Split(segment, "-")
			localizedTokens := make([]string, 0, len(tokens))
			allMapped := true
			for _, token := range tokens {
				mappedToken, ok := fallbackTokenMap[token]
				if !ok {
					allMapped = false
					break
				}
				localizedTokens = append(localizedTokens, mappedToken)
			}
			if allMapped && len(localizedTokens) > 0 {
				segments = append(segments, strings.Join(localizedTokens, ""))
				continue
			}
		}
