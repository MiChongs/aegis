...
`ReplayGuard` 对整个 `/api/v1/apps/` 前缀让开（登记在 `replay_policy.go` 的策略表里）：
...
| 层 | 文件 | 触发条件 |
|---|---|---|
| 1 路由策略 | `replay_policy.go` | 恒生效，决定后面三层要不要跑 |
| 2 Nonce + 签名 | `replay_guard.go` | 请求带 `X-Timestamp` + `X-Nonce` |
| 3 幂等键 | `idempotency.go` | 请求带 `Idempotency-Key` |
| 4 指纹兜底 | `replay_guard.go` | 仅 `PolicyGuarded` 且没带幂等键 |

### 为什么把「默认拦截」改成了「默认放行」

旧实现只有第 4 层，且**对所有非 GET 请求生效**：算一次
`method + path + IP + token 前缀 + body 前 2KB` 的哈希，5 秒内重复就回 403。

它的隐含假设是「两次一模一样的请求必然是重复提交」，而这个假设是错的。
大量接口天生就要被原样重复调用：**点一次「换一张」发出的就是一模一样的验证码请求**，
于是第二次点击稳定收到 403「重复请求」。短信重发、同一个错码再试一次、
样张预览、POST 检索、用同一个错密码再登录一次，全是同一类。

而它对真正要防的东西又几乎无效：攻击者改一个字节就绕过去了，
用户在第 6 秒重复点一次「创建订单」照样能创建两单。
也就是说这条规则**误伤了它不该管的，又没挡住它该挡的**。

现在判据从「猜」改成「声明」，未登记的路由默认 `PolicyIdempotent`（不做指纹去重）。
新增接口时忘记登记，后果是「少了一层兜底」而不是「这个接口在生产上随机 403」——
前者能靠 review 发现，后者只会变成一张没人能复现的工单。

**匹配单位是路径段，不是子串。** `strings.Contains(path, "/captcha")` 会把
`/api/admin/apps/:appkey/captcha-config` 一起放行，而那是个写配置的接口。

### 幂等键：重复请求要拿到「第一次的那个响应」

回 403 对调用方毫无用处：一次超时重试拿到 403，他仍然不知道第一次到底成没成功，
只能再试或人工核对。有用的回答是把首次的响应原样还给他，他才能继续往下走。
这套语义就是 Stripe / Square 那一套，也是 IETF
`draft-ietf-httpapi-idempotency-key-header` 正在标准化的东西：

| 情形 | 响应 |
|---|---|
| 首次 | 正常执行，响应存档（TTL 默认 24h） |
| 同键同内容 | 回放存档的响应 + `Idempotent-Replayed: true` |
| 同键不同内容 | 422 `40384`，明说这个键已用于另一份请求体 |
| 正在执行中 | 409 `40386` + `Retry-After` |
| 键格式非法 | 400 `40387` |

三层并发控制，从便宜到贵：`singleflight`（同进程合流）→ `redislock`
（跨实例执行中标记）→ Redis 记录（已完成的响应快照）。
用 redislock 而不是裸 `SetNX`，是因为释放锁必须校验持有者令牌 ——
裸 `SetNX` + `DEL` 会在超时后删掉**别人**的锁。

四条踩过的坑：

1. **跟随者必须自己产出响应。** singleflight 只负责选出「谁去执行」，
   领跑者写的是**它自己**那个 ResponseWriter。跟随者若只是拿到一个
   `handled=true` 就从中间件 return，Gin 会继续往下走 ——
   **一个中间件不调用 `c.Abort()` 就等于放行**。表现是 singleflight 明明只跑了一次，
   handler 却执行了四遍。现在跟随者会去读领跑者存下的记录并回放。
2. **5xx 不存档。** 服务端自己出错时重试就该真的重跑一遍，
   把一次 500 缓存 24 小时等于把偶发故障钉死成永久故障。
3. **作用域必须含调用方。** 只用键本身做 key，两个调用方碰巧用了同一个 UUID
   时后来者会拿到前者的响应，那是一次跨账号的数据泄露。
4. **释放锁与存档用 `context.WithoutCancel`。** 客户端断开会取消请求 ctx，
   那时锁还没释放，同一个键要等到 lockTTL 到期才能再用。

### 签名必须先于 Nonce 消耗

旧实现是反的：先 `TryAcquireNonce` 再验签。于是一个签名错误的请求会把 Nonce 先烧掉，
客户端拿正确签名重试时收到「重复请求」—— 而它其实一次都没成功过。
更糟的是任何人只要猜到别人正在用的 Nonce，就能用一个乱签名把它作废。
`TestBadSignatureDoesNotBurnNonce` 钉住这一条。

### 指纹失败后要归还占位

指纹是在 handler **执行前**占下的。这次请求若最终 4xx，它并没有产生副作用，
占位就该立刻还回去 —— 否则用户改完参数重试会撞上自己刚才那次失败，
而错误信息是「重复请求」，与他真正遇到的问题毫无关系。

### 拒绝一律用 409，不用 403

403 会把调用方引向排查凭据，而正确的动作是稍后重试或去查询结果。
所有重复类拒绝都带 `Retry-After`。
